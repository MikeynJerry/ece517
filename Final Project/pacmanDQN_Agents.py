...
class PacmanDQN(game.Agent):
    ...
    def observation_step(self, state):
        if self.last_action is not None:
            ...
            if reward > 20:
                self.last_reward = 50.0  # Eat ghost   (Yum! Yum!)
            elif reward > 0:
                self.last_reward = 10.0  # Eat food    (Yum!)
            elif reward < -10:
                self.last_reward = -500.0  # Get eaten   (Ouch!) -500
                self.won = False
            elif reward < 0:
                self.last_reward = -1.0  # Punish time (Pff..)
